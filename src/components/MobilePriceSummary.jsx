...
const MobilePriceSummary = ({
  ...
  // VAT is included in price (VAT-inclusive). Compute VAT portion as price * (10 / 110).
  const computedVat = safeSelectedRooms.reduce((sum, r) => {
  ...
  (selectedServices?.reduce((sum, s) => {
    ...
    const vat = s?.vat !== undefined ? Number(s.vat) : price * (10 / 110);
    return sum + vat;
  }, 0) || 0);
  safeSelectedRooms.forEach(r => console.log(`Room ${r.roomName}: price ${r.price}, vat ${r.vat}`));
  selectedServices?.forEach((s, idx) => {
  });
  // If you want to always use context, you could replace with:
  // const { selectedRooms, removeRoom } = useSelectedRooms();
  // const safeSelectedRooms = Array.isArray(selectedRooms) ? selectedRooms : [];

  const computedTotalPrice = safeSelectedRooms.reduce((sum, r) => sum + (r.price || 0), 0) +
  ...
  // Improved guest allocation logic (effect-based, updates when guests or rooms change)
  const [canAccommodate, setCanAccommodate] = useState(false);

  useEffect(() => {
    if (!safeSelectedRooms.length) {
      setCanAccommodate(false);
      return;
    }
    ...
    const sortedRooms = [...safeSelectedRooms].sort((a, b) => {
      const aCap = a.maxAdults || a.capacity?.maxAdults || 0;
      const bCap = b.maxAdults || b.capacity?.maxAdults || 0;
      return bCap - aCap;
    });

    for (const room of sortedRooms) {
      const maxAdults = room.maxAdults ?? room.capacity?.maxAdults ?? 0;
      const maxChildren = room.maxChildren ?? room.capacity?.maxChildren ?? 0;
      const maxTotal = room.maxTotal ?? room.capacity?.maxTotal ?? (maxAdults + maxChildren);

      const useAdults = Math.min(maxAdults, remainingAdults);
      remainingAdults -= useAdults;

      const totalInRoom = useAdults;
      const remainingSpace = maxTotal - totalInRoom;
      const useChildren = Math.min(maxChildren, remainingChildren, remainingSpace);
      remainingChildren -= useChildren;
    }

    setCanAccommodate(remainingAdults <= 0 && remainingChildren <= 0);
  }, [safeSelectedRooms, localAdults, localChildren]);
  ...
  return (
    <div className="fixed inset-0 bg-black/30 z-50 sm:hidden flex flex-col">
      <motion.div
        ...
        className="bg-white rounded-t-2xl p-6 relative flex flex-col h-full overflow-y-auto"
      >
        {/* Close Button */}
        <button
          onClick={onConfirm}
        ...
        <div className="bg-white px-1 py-1 w-full mt-auto border-t border-gray-200">
          <div className="flex justify-between items-center mb-4">
            <div className="text-base font-bold text-gray-900">
              TOTAL <span className="font-normal text-gray-500 text-xs">(fees and taxes included)</span>
            </div>
            <div className="text-green-700 font-extrabold text-lg">
              {formatCurrency(computedTotalPrice, exchangeRate, currencyCode)}
            </div>
          </div>
          {!location.pathname.includes("/guest-info") && (
            <>
              <button
                onClick={() => {
                  ...
                  if (!canAccommodate) {
                    ...
                    setErrorMessage(`Add more room(s) to fit ${totalGuests} guest${totalGuests > 1 ? "s" : ""}.`);
                    return;
                  }
                  setErrorMessage("");
