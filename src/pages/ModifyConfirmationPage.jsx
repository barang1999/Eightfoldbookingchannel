...
import TotalWithVatSummary from "../components/TotalWithVatSummary";
import Header from "../components/Header";
import MobileStepper from "../components/MobileStepper";
import BookingSummaryBox from "../components/BookingSummaryBox";
import ModifyMobileBookingSummaryBox from "../components/ModifyMobileBookingSummaryBox";
import SupportButton from "../components/SupportButton";
import { useNavigate, useLocation } from "react-router-dom";
import ModifyPriceSummary from "../components/ModifyPriceSummary";
import MobilePriceSummary from "../components/MobilePriceSummary";
import ModifyMobilePriceSummary from "../components/ModifyMobilePriceSummary";
...
const ModifyConfirmationPage = ({ propertyId: passedPropertyId }) => {
  const isMobile = useMediaQuery({ maxWidth: 639 });
  const propertyId = passedPropertyId || fallbackPropertyId;
  const location = useLocation();
  const passedState = location.state || {};
  const reservationId =
    passedState.reservationId ||
    localStorage.getItem("reservationId") ||
    "";
  console.log("🧾 Loaded Reservation ID:", reservationId);
  const overrideRange = passedState.dateRange || {};
  const overrideGuests = passedState.guests || {};
  const overrideTotalGuests = passedState.totalGuests || 0;
  const navigate = useNavigate();
  useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  const { selectedRooms } = useSelectedRooms();
  // Enrich selectedRooms with guaranteed roomName for rendering
  // [📦 Raw SelectedRooms] Log raw value from context before enriching
  const enrichedRooms = selectedRooms.map(room => ({
    ...room,
    roomName: room.roomName || room.name || "Restored Room"
  }));
  ...
  // Override range and guests from navigation state if present
  if (overrideRange.checkIn && overrideRange.checkOut) {
    range = { from: overrideRange.checkIn, to: overrideRange.checkOut };
  }

  const guestAdults = overrideGuests.adults ?? (Number(localStorage.getItem("numAdults")) || 0);
  const guestChildren = overrideGuests.children ?? (Number(localStorage.getItem("numChildren")) || 0);
  ...
  const nights =
    checkInDate && checkOutDate && !isNaN(checkInDate) && !isNaN(checkOutDate)
      ? Math.max((checkOutDate - checkInDate) / (1000 * 60 * 60 * 24), 1)
  ...
  const storedGuestInfo = JSON.parse(localStorage.getItem("modifyGuestInfo") || "{}");
  const rawNumAdults = localStorage.getItem("numAdults");
  const rawNumChildren = localStorage.getItem("numChildren");
  // guestAdults and guestChildren are now overridden above if overrideGuests present
  ...
  const handleModifyConfirm = async () => {
    ...
    // --- [Use roomsWithRates directly] ---
    const finalRooms = roomsWithRates;
    // Build bookingData object before API call
    const bookingData = {
      propertyId,
      ...
      numberOfGuests: `${guestAdults} adult${guestAdults > 1 ? "s" : ""}${guestChildren > 0 ? `, ${guestChildren} child${guestChildren > 1 ? "ren" : ""}` : ""}`,
      specialRequest: storedGuestInfo.specialRequest || guestInfo.specialRequest || localStorage.getItem("specialRequest") || "",
      ...
      modification: true,
      firebaseUid,
      totalBeforeTax: summaryTotalBeforeTax,
      taxesAndFees: summaryTaxesAndFees,
      totalAfterTax: summaryTotalAfterTax,
      rooms: finalRooms
    };

    // Debugging: log bookingData and roomsWithRates snapshot before submission
    console.log("🛠️ Final bookingData before submission:", JSON.stringify(bookingData, null, 2));
    console.log("🧾 Final roomsWithRates snapshot:", roomsWithRates.map(r => ({
      roomId: r.roomId,
      baseRate: r.baseRate,
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_BOOKING_API_URL}/modify/${reservationId}`, {
        method: "PUT",
        headers: {
      ...
      if (result.success) {
        ...
        navigate(`/modification-success/${result.booking._id}`, { replace: true });
      } else {
        alert("Modification failed: " + result.message);
      }
    } catch (err) {
      console.error("Modification error:", err);
      alert("An error occurred while confirming your modification.");
    } finally {
  ...
  // Track when all room details have loaded
  const [roomDetailsReady, setRoomDetailsReady] = useState(false);

  useEffect(() => {
    setRoomDetailsReady(false);
    let completed = 0;
  ...
  // Responsive grid
  // [🐛 Room Visibility Check] Log enrichedRooms length before render
  // Construct bookingData here so it's available when roomDetailsReady is true
  const guestInfoForBooking = JSON.parse(localStorage.getItem("guestInfo") || "{}");
  const selectedBedsForBooking = JSON.parse(localStorage.getItem("selectedBeds") || "{}");
  const auth = getAuth();
  const currentUser = auth.currentUser;
  const firebaseUid = currentUser?.uid || null;
  // Ensure all required fields are present in bookingData
  const resolvedFullName =
    guestInfoForBooking.fullName ||
    storedGuestInfo.fullName ||
    guestInfoForBooking.name ||
    storedGuestInfo.name ||
    `${guestInfoForBooking.firstName || storedGuestInfo.firstName || ""} ${guestInfoForBooking.lastName || storedGuestInfo.lastName || ""}`.trim();
  const resolvedEmail = guestInfoForBooking.email || guestInfoForBooking.contactEmail || storedGuestInfo.email;
  const resolvedPhone =
    guestInfoForBooking.phone ||
    storedGuestInfo.phone ||
    (guestInfoForBooking.countryCode && guestInfoForBooking.phone ? `${guestInfoForBooking.countryCode} ${guestInfoForBooking.phone}` : "");
  // Debug log for roomDetailsMap before mapping rooms
  // console.log("📦 Room details available:", roomDetailsMap);
  const roomsWithRates = enrichedRooms.map((room) => {
    ...
    const baseRate =
      ...
      room.roomOnlyRate ||
      room.price ||
      details.basePriceRoomOnly ||
      details.promotionRoomOnlyRate ||
      details.roomOnlyRate ||
      details.price ||
      0;
    const vat = propertyData?.policy?.vat?.percentage || 10;
    const totalBeforeTax = baseRate;
    const taxesAndFees = baseRate * vat / (100 + vat);
    const totalAfterTax = baseRate; // VAT already included
    ...
    // --- [🛏️ Backend Bed Info Logic] ---
    const instanceBed = selectedBeds[room.instanceId];
    let bedType = "";
    let bedCountLabel = "";

    if (room.requiresBedChoice && instanceBed) {
      bedType = instanceBed;
      if (instanceBed.includes("double") && details.doubleBedCount) {
        bedCountLabel = `${details.doubleBedCount} large double bed${details.doubleBedCount > 1 ? "s" : ""}`;
      } else if (instanceBed.includes("single") && details.singleBedCount) {
        bedCountLabel = `${details.singleBedCount} single bed${details.singleBedCount > 1 ? "s" : ""}`;
      } else {
        bedCountLabel = instanceBed;
      }
    } else if (details?.fixedBedSetup?.length > 0) {
      bedType = details.fixedBedSetup.join(" + ");
      const parts = [];
      if (details.doubleBedCount) parts.push(`${details.doubleBedCount} large double bed${details.doubleBedCount > 1 ? "s" : ""}`);
      if (details.singleBedCount) parts.push(`${details.singleBedCount} single bed${details.singleBedCount > 1 ? "s" : ""}`);
      bedCountLabel = parts.join(" + ");
    } else {
      bedType = room.bedType || "";
      bedCountLabel = room.bedCountLabel || "";
    }

    console.log(`[🛏️ Backend Bed Info] Room: ${room.roomName}, Bed Type: ${bedType}, Count: ${bedCountLabel}`);
  ...
  // Calculate summary rates for the whole booking
  const summaryTotalBeforeTax = roomsWithRates.reduce((sum, r) => sum + (r.totalBeforeTax || 0), 0);
  const summaryTaxesAndFees = roomsWithRates.reduce((sum, r) => sum + (r.taxesAndFees || 0), 0);
  const summaryTotalAfterTax = roomsWithRates.reduce((sum, r) => sum + (r.totalAfterTax || 0), 0);
  

  // Show a preview of bookingData as soon as it's ready
...
if (!roomDetailsReady) {
  ...
  return (
    ...
    >
      ...
      {/* Progress Stepper */}
      <div className="max-w-7xl mx-auto px-4 pt-4 block">
        {isMobile ? (
          <PageNavigation
            items={[
              ...(reservationId
                ? [{ label: "Modify Reservation", href: `/modify-reservation/${reservationId}` }]
                : []),
      ...
      <div className="max-w-7xl mx-auto pt-2 px-4 lg:px-6">
        ...
        <h2 className="text-2xl font-bold mb-5 text-center lg:text-left">Modification Confirmation</h2>
              ...
              <div className="flex flex-col gap-4">
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
                  ...
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <span className="font-semibold text-sm">{enrichedRooms.length}</span>
                  </div>
                </div>
              </div>
            </div>

            {/* Booking Summary Box (Mobile only, always mounted) */}
            <div className="mb-2 lg:hidden">
              {(() => {
                console.log("🧭 ModifyMobileBookingSummaryBox Props", {
                  ...
                  selectedRooms: enrichedRooms,
                });
                return null;
              })()}
              <ModifyMobileBookingSummaryBox
                hotel={propertyData || hotel}
                stayPeriod={{
                  start: range?.from,
                  end: range?.to,
                ...
                guestCount={{
                  adults:
                    typeof overrideGuests.adults !== "undefined"
                      ? Number(overrideGuests.adults)
                      : Number(localStorage.getItem("numAdults") || guestAdults || 0),
                  children:
                    typeof overrideGuests.children !== "undefined"
                      ? Number(overrideGuests.children)
                      : Number(localStorage.getItem("numChildren") || guestChildren || 0),
                }}
                selectedRooms={enrichedRooms}
              />
            </div>
            ...
            {isLoading ? (
              ...
              <>
                {enrichedRooms.map((room, index) => {
                  // [📦 Rendering Room {index}]
                  // 🧩 Debug pricing info for this room card
                ...
                <div className="flex flex-col gap-1 pl-1">
                  <div className="flex items-center gap-1">
                    ...
                    <span className="font-medium text-sm text-gray-700">
                      {storedGuestInfo.name || guestInfo.fullName || `${guestInfo.firstName || ""} ${guestInfo.lastName || ""}`.trim() || "—"}
                    </span>
                  </div>
                  <div className="flex items-center gap-1">
                    <Mail className="w-4 h-4 text-gray-400" />
                    <span className="text-gray-500 text-xs font-medium">Email:</span>
                    <span className="font-medium text-sm text-gray-700">{storedGuestInfo.email || guestInfo.email || "—"}</span>
                  </div>
                  <div className="flex items-center gap-1">
                    <PhoneCall className="w-4 h-4 text-gray-400" />
                    <span className="text-gray-500 text-xs font-medium">Phone:</span>
                    <span className="font-medium text-sm text-gray-700">
                      {storedGuestInfo.phone || (guestInfo.phone ? `${guestInfo.countryCode || ""} ${guestInfo.phone}` : "—")}
                    </span>
                  </div>
                  {/* Total Guests */}
                  ...
                  {/* Arrival Time */}
                  {(storedGuestInfo.arrivalTime || localStorage.getItem("arrivalTime")) && (
                    <div className="flex items-center gap-1">
                  ...
                  {/* Special Requests */}
                  {(storedGuestInfo.specialRequest || localStorage.getItem("specialRequest")) && (
        ...
        <div className="lg:hidden mt-8">
          ...
          />
            ...
            <div className="flex justify-center mt-8">
              <button
                ...
                disabled={isSubmitting || !roomDetailsReady}
              >
                {isSubmitting ? (
                  <>
          ...
          <div className="hidden sm:block w-[480px] pr-4 pt-2 sticky top-6 self-start bg-white rounded-2xl shadow-md p-6 border border-gray-100">
      ...
      <div className="fixed right-0 bottom-4 sm:bottom-8 z-30">
        <SupportButton propertyId={propertyId} className="relative" />
      </div>
...
export default ModifyConfirmationPage;